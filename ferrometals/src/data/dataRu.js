...
const dataRu = {
    ...
    booklets:
             ...
             polymerPaintingLines: 
               ...
               path: "FerroMetals_Polymer_painting_lines_RU.pdf", 
               title: "FerroMetals_Polymer_painting_lines_RU.pdf"
               },
             palletEquipment: 
    ...
    menuItems: 
            ...
            polymerPaintingLines: "Линии полимерного покрытия",
    ...
    polymerPaintingLines: 
                ...
                mainText: `Линия может иметь кольцевое и полукольцевое исполнение. В кольцевом исполнении имеется возможность сквозного движения 
                заготовок через камеры, что позволяет улучшить логистику в отношении окрашенных и неокрашенных деталей вследствие 
                постоянства точек загрузки/разгрузки и направлений перемещения, применяется для крупносерийного непрерывного 
                производства. При наличии требований к максимальной компактности линии, предлагается полукольцевая компоновка, при 
                которой камеры имеют тупиковое исполнение, а транспортная система предполагает возвратно-поступательное движение 
                заготовок между камерами.`,
                internalTitleText: `В состав линии входит:`,
                internalBlocksText: 
                                [{id: 0,
                                header: `1. Камера полимеризации с автоматизированной системой управления.`,
                                subHeader: `Конструкция камеры предусматривает:`,
                                blockText:
                                        [`теплоизолированный корпус с отделкой внутренней поверхности оцинкованным стальным 
                                        листом;`,
                                        `термоблок с оребренными ТЭНами из нержавеющей стали и системой принудительной циркуляции 
                                        воздуха со специальным термостойким вентилятором. Внутренняя поверхность воздуховода отделана 
                                        нержавеющей сталью для предотвращения появления окалины и последующего загрязнения окрашиваемых 
                                        изделий;`,
                                        `полнофункциональный блок управления с сенсорной панелью и удобным графическим интерфейсом 
                                        для возможности работы в полностью автоматическом режиме (нагрев – выдержка – отключение 
                                        нагрева – принудительная продувка (охлаждение) отключенных ТЭНов);`,
                                        `мощные петли и каркас камеры, исключающие возможность провисания дверей и нарушения 
                                        геометрии корпуса;`,
                                        `простая электрическая схема с использованием стандартных компонентов обеспечивает высокую 
                                        ремонтопригодность;`,
                                        `эксплуатируемый пол: ТЭНы расположены в воздуховодах термоблоков за пределами внутреннего 
                                        габарита камеры, что позволяет при необходимости размещать изделия, которые невозможно 
                                        подвесить, на специальных подставках на полу;`,
                                        `система принудительной циркуляции воздуха, обеспечивающая равномерное перемешивание воздуха 
                                        и стабилизацию температуры по всему рабочему объему камеры в пределах 5ᵒС;`,
                                        `вытяжной зонт для удаления газообразных выделений во время открывания двери камеры в конце 
                                        цикла.`]
                                },
                                {id: 1,
                                header: `2. Камера нанесения порошка в электростатическом поле, укомплектованная высокопроизводительной 
                                ручной установкой нанесения порошковых красок и системой приточно-вытяжной рекуперации не 
                                задействованного порошка.`,
                                subHeader: `Конструкция камеры предусматривает:`,
                                blockText:
                                        [`нахождения оператора внутри камеры во время процесса окраски (камера обитаемого типа);`,
                                        `стенки камеры из прозрачного диэлектрического полимера;`,
                                        `отделка пола оцинкованным листом для удобства удаления осевшего порошка;`,
                                        `светодиодное освещение рабочей зоны;`,
                                        `распашные либо сдвижные двери (по желанию заказчика);`,
                                        `систему рекуперации, предназначенную для вытяжки воздуха из камеры напыления, его фильтрации, 
                                        сбора не осевшего на поверхность окрашиваемых деталей полимерного порошка в специальном бункере, 
                                        с целью его возврата в технологический цикл. Используются 2 ступени очистки: в качестве первой 
                                        ступени применяется циклон, вторая ступень – картриджный полиэстеровый фильтр. Степень очистки 
                                        воздуха – до 99,9%.`]
                                },
                                {id: 2,
                                header: `3. Транспортная система на основе верхних подвесных многоручьевых рельсовых направляющих и 
                                комплекта роликовых кареток-подвесов для крепления заготовок.`,
                                subHeader: `Конструкция камеры предусматривает:`,
                                blockText:
                                        [`высокую несущую способность и точность стыковки за счет использования высокопрочного 
                                        прокатного закрытого профиля для направляющих – масса единичной заготовки до 500 кг;`,
                                        `легкое ручное перемещение кареток с заготовками за счет использования шарнирных подвесов на 
                                        подшипниках качения с термостойкой графито-наполненной смазкой;`,
                                        `путь, проходящий сквозь камеру нанесения, расположен в воздушном затворе для предотвращения 
                                        накопления порошка на поверхности подвесов;`,
                                        `модульная разборная конструкция, позволяющая изменять и дополнять конфигурацию при 
                ...
                pictures: 
                        ...
                        parameters: 
                                [{id: 0,
                                parameterName: 'Состав базового комплекта линии полимерной окраски.',
                                parameterValue: ''},
                                {id: 1,
                                parameterName: 'Наименование оборудования:',
                                parameterValue: ''},
                                {id: 2,
                                parameterName: '1. Камера нанесения порошковой полимерной краски в электростатическом поле:',
                                parameterValue: ''},
                                {id: 3,
                                parameterName: 'Тип камеры',
                                parameterValue: 'проходная'},
                                {id: 4,
                                parameterName: 'Привязка к транспортной системе',
                                parameterValue: 'верхняя'},
                                {id: 5,
                                parameterName: 'Параметры электропитания, В / Гц',
                                parameterValue: '380 / 50'},
                                {id: 6,
                                parameterName: 'Внутренний габарит камеры, мм',
                                parameterValue: '8000 х 2000 х 3000'},
                                {id: 7,
                                parameterName: 'Емкость контейнера для порошка, кг',
                                parameterValue: '40'},
                                {id: 8,
                                parameterName: 'Общая мощность, кВт',
                                parameterValue: '8'},
                                {id: 9,
                                parameterName: 'Укомплектована установкой порошкового напыления в электростатическом поле и системой подачи чистого воздуха к шлему оператора',
                                parameterValue: ''},
                                {id: 10,
                                parameterName: '2. Транспортирующая система:',
                                parameterValue: ''},
                                {id: 11,
                                parameterName: 'Тип транспортера',
                                parameterValue: 'закольцованный подвесной реверсивный'},
                                {id: 12,
                                parameterName: 'Тип тягового элемента',
                                parameterValue: 'цепь тяговая пластинчатая с траверсами'},
                                {id: 13,
                                parameterName: 'Тип трассы',
                                parameterValue: 'ходовая двутавровая балка'},
                                {id: 14,
                                parameterName: 'Шаг грузозахватных траверс, мм',
                                parameterValue: '400'},
                                {id: 15,
                                parameterName: 'Общая грузоподъемность, кг',
                                parameterValue: '1500'},
                                {id: 16,
                                parameterName: 'Мощность привода, кВт',
                                parameterValue: '3'},
                                {id: 17,
                                parameterName: 'Скорость движения,  м/с',
                                parameterValue: '0 – 250'},
                                {id: 18,
                                parameterName: 'Длина трассы, м',
                                parameterValue: '35'},
                                {id: 19,
                                parameterName: '3. Камера полимеризации:',
                                parameterValue: ''},
                                {id: 20,
                                parameterName: 'Тип камеры',
                                parameterValue: 'проходная'},
                                ...
                                parameterValue: 'верхняя'},
                                {id: 22,
                                parameterName: 'Общая мощность ТЭНов, кВт',
                                parameterValue: '200'},
                                {id: 23,
                                parameterName: 'Режим работы',
                                parameterValue: 'автоматический'},
                                {id: 24,
                                parameterName: 'Время выхода на рабочий режим при максимальной загрузке, мин',
                                parameterValue: '40'},
                                {id: 25,
                                parameterName: 'Максимальная температура нагрева, С°',
                                parameterValue: '230'},
                                {id: 26,
                                parameterName: 'Параметры электропитания, В / Гц',
                                parameterValue: '380 / 50'},
                                {id: 27,
                                parameterName: 'Напряжение на ТЭНах, В',
                                parameterValue: '220'},
                                {id: 28,
                                parameterName: 'Количество термоблоков, шт',
                                parameterValue: '4'},
                                {id: 29,
                                parameterName: 'Внутренний габарит камеры, мм',
