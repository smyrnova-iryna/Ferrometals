...
const PolymerCoatingLines = ({data}) => {
    return (
        <section className="Standart-Section Product-Container">
            <h3 className="Product-Heading">{data.polymerCoatingLines.mainHeader}</h3>
            <article className="Product-Info-Container">
                <div className="Product-First-Info-Container">
                    <img className="Scaner-Image" 
                    src={require(`../../data/picture/picture_products/${data.polymerCoatingLines.mainPictures[0].picture}`)} 
                    alt={data.polymerCoatingLines.mainPictures[0].captionText} />
                    <p className="Scaner-Text">{data.polymerCoatingLines.mainText[1]}</p>
                </div>
                <div className="Scaner-Decorated-Left-Text-Container">
                    <p className="Scaner-Decorated-Left-Text">{data.polymerCoatingLines.mainText[2]}</p>
                </div>
                <div className="Scaner-Decorated-Right-Text-Container">
                    <svg className="Pulse-Icon" fill="rgba(20,172,147,255)" height="80px" width="80px" version="1.1" id="Layer_1" 
                    viewBox="0 0 511.999 511.999" stroke="rgba(20,172,147,255)">
                    ...
                    <div className="Product-Item-Container" key={item.idGlobal}>
                        <div className="Product-Content-Container" style={{backgroundColor: "var(--main-background-color)"}}>
                            ...
                            <div className="Product-Description-Container">
                                {data.polymerCoatingLines.pictures[item.idGlobal].parameters.map((localItem) => 
            ...
            <div className="Product-Options-Container">
                <h4 className="Product-Options-Title">{data.booklets.polymerCoatingLines.product}</h4>
                <img className="Product-Options-Image" alt={data.polymerCoatingLines.mainHeader}
                ...
                download={data.booklets.polymerCoatingLines.title}>{data.booklets.title}</a>
            </div>
            {/* <div className="About-Text-Container">
                <a href={require(`../../booklets/${data.booklets.polymerCoatingLines.path}`)} 
                download={data.booklets.polymerCoatingLines.title}>{data.booklets.title}</a>
            </div> */}
        </section>
    )
...
export default PolymerCoatingLines;